...
- можно ли доверять `header.stamp` или нужно автоматически перейти на timing по времени получения сообщения;
...
- `timing_source` — откуда взяты интервалы: из `header.stamp` или из времени получения сообщения.

Если во время bag playback или в нестабильном upstream-пайплайне `header.stamp` начинает повторяться либо идти назад, `camera_slam_preflight` автоматически переключается на timing по времени получения сообщения. Это не скрывает проблему: узел пишет предупреждение и показывает счётчики `missing_source_stamps` / `non_monotonic_source_stamps` в summary.
...
- даже fallback по времени получения показал слишком низкий FPS;
