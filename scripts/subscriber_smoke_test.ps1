...
Write-Host '[ИНФО] Запускается image_counter для smoke-теста.'
...
try {
  ...
  Write-Host '[ИНФО] Публикуется синтетическое изображение в /camera/image_raw.'
  ...
  if ($LASTEXITCODE -ne 0) {
    throw 'Не удалось опубликовать тестовое изображение.'
  }

  if (-not $process.WaitForExit(20000)) {
    throw 'image_counter не завершился после получения тестового кадра.'
  }
  ...
  if ($combinedOutput -notmatch 'frame_id=test_camera width=1 height=1 encoding=bgr8 step=3') {
    throw "Smoke-тест не подтвердил получение кадра. Подробности: $stderrFile"
  }
